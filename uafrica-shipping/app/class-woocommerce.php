...
class WooCommerce {
	...
	public static function add_suburb_to_checkout_fields( $fields ) {
		...
		$suburb = [
			'label'    => 'Suburb',
			'class'    => [ 'form-row-wide', 'address_field' ],
			'priority' => 65,
		];

		$fields['shipping']['shipping_suburb'] = $suburb;
		$fields['billing']['billing_suburb']   = $suburb;
	...
	public static function modify_shipping_packages( $packages ) {
		...
		foreach ( $packages as $index => $package ) {
			// Add shipping suburb to destination address
			$packages[$index]["destination"]["shipping_suburb"] = WC()->customer->get_meta( 'shipping_suburb' );
